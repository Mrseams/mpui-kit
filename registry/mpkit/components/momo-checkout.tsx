...
import type { MethodPanel } from "@/components/mpkit/method-panel"
...
import { ReceiptCard, type ReceiptDetail } from "@/components/mpkit/receipt-card"
import { UssdPrompt } from "@/components/mpkit/ussd-prompt"
import { useMomoCheckout, type UseMomoCheckoutOptions } from "@/hooks/mpkit/use-momo-checkout"
import type { CheckoutStatus, PaymentReceipt } from "@/lib/mpkit/checkout-machine"
...
export interface MomoCheckoutProps
  ...
  /**
   * (Beta) Your own panel for a payment method, by method id: where you host your
   * payment provider's card fields or buttons (Stripe, PayPal, ...). It replaces
   * the built-in content for that method. Card details never enter MP Kit;
   * the panel hands back an opaque token that reaches `onPay` as `payload`.
   * See `MethodPanel`.
   */
  panels?: Record<string, MethodPanel>
  /** Your own icons by method id, for example a wallet's logo. MP Kit ships no brand logos. */
